...
class TicTacToe {
    ...
    private validateRuneSet(runeSet: string) {
        ...
        if ((runeSet.match(validRuneRegex) || []).length < (this.boardSize ^ 2)) {
            throw new Error(`board map invalid - bad runes found - only ${this.runes['one']}, ${this.runes['two']}, and ${this.runes['empty']} are permitted in board map`)
        }
